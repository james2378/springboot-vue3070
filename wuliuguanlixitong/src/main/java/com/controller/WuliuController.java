    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(wuliu !=null){
            ...
            BeanUtils.copyProperties( wuliu , view );//把实体数据重构到view中
                ...
                //级联表
                LuxianEntity luxian = luxianService.selectById(wuliu.getLuxianId());
                if(luxian != null){
                    BeanUtils.copyProperties( luxian , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段
    ...
    */
    ...
    public R save(@RequestBody WuliuEntity wuliu, HttpServletRequest request){
        ...
        Wrapper<WuliuEntity> queryWrapper = new EntityWrapper<WuliuEntity>()
            ...
            .eq("luxian_id", wuliu.getLuxianId())
...
//                            wuliuEntity.setLuxianId(Integer.valueOf(data.get(0)));   //路线 要改的
        ...
        WuliuEntity wuliu = wuliuService.selectById(id);
            if(wuliu !=null){
                ...
                //级联表
                    YundanEntity yundan = yundanService.selectById(wuliu.getYundanId());
                if(yundan != null){
                    BeanUtils.copyProperties( yundan , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setYundanId(yundan.getId());
                }
                //修改对应字典表字段
                dictionaryService.dictionaryConvert(view, request);
